...
void Extruder_One_Rev2_Cmd_Handler(void){
	...
	if(!strcmp(Command_Str, "HELLO")){	
		...
		sprintf(Response_Buffer,"1 OK HELLO TYPE:EXTRUDER ID:%08X%08X%08X VENDOR:%s FIRMWARE:%s VERSION:%s EXTRUDER:1R2 MAX_TEMPERATURE:%.1lf USED:%u ",UUID[2],UUID[1],UUID[0],Vender,Firmware_Name,Firmware_Version,Max_Temperature,Read_Using_Time());
