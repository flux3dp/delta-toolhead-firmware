...
void Unknow_Module_Cmd_Handler(void){
	...
	if(!strcmp(Command_Str, "HELLO")){		
		...
		sprintf(Response_Buffer,"1 OK HELLO TYPE:UNKNOW ID:%08X%08X%08X VENDOR:%s FIRMWARE:%s VERSION:%s ",UUID[2],UUID[1],UUID[0],Vender,Firmware_Name,Firmware_Version);
